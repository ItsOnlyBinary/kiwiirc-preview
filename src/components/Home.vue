<template>
    <div>
        ...
        <table v-if="json.repos != null && json.users != null">
            <tr :key="user.id">
                <td colspan="5" class="table-title"><a :href="'https://github.com/' + user.nane + '/kiwiirc/'">{{ user.title }}</a></td>
            </tr>
            <tr :key="user.id">
                <th>branch</th>
                <th>github sha</th>
